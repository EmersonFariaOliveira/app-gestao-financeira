...
**Identidade de "ativo sob ajuste":** não há flag própria de ativação/desativação — um `chave_export` está "sob ajuste" a partir do momento em que existe ao menos uma linha `ajuste_valor_investido` para ele (criada na tela de gestão dedicada, fora do fluxo de import, análoga ao "+ Nova posição manual" da seção 6.9 — decisão de UI fora do escopo deste documento). "Ajuste ativo" para fins de exibição/criação/edição (tela dedicada `listarAjustesAtivos` e carry-forward `ajustesRevisao` de `montarRevisaoImport`, FR-007) é uma condição **derivada**: existe histórico de `ajuste_valor_investido` para o `chave_export` **E** `ativo_mapeado.ignorar_no_import = false` **E** (`ativo_mapeado.alvo_id IS NOT NULL` **OU** `ativo_mapeado.fora_da_carteira = true`). Em outras palavras, um `chave_export` "pendente" (sem alvo e não fora-da-carteira) fica de fora, mas um `chave_export` marcado `fora_da_carteira = true` (feature "vínculos fora da carteira") PODE ter um ajuste, mesmo sem `alvo_id` — nesse caso `alvoId`/`nomeAlvo` retornam `null` na saída (não se força um alvo fictício).
Isso cobre o edge case do spec.md ("ajuste cujo ativo é desvinculado do alvo": o histórico permanece auditável, mas ele para de aparecer no carry-forward quando o ativo é ignorado — FR-015).

**Distinto da elegibilidade de incremento automático.** A identidade acima (quem PODE ter um ajuste exibido/criado) é **deliberadamente mais ampla** do que a fórmula `elegíveis` da seção "Elegibilidade para incremento automático" abaixo, que continua exigindo `alvo_id` (um aporte é sempre feito NUM alvo — um `chave_export` fora-da-carteira, sem alvo, nunca é alvo de incremento automático por aporte executado). Na prática, para uma chave fora-da-carteira sem alvo, `incrementoPendenteCentavos` sempre resolve para `0` no carry-forward, porque nunca existe uma linha `incremento_valor_investido_pendente` associada a ela.
